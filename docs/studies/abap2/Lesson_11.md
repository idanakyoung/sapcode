...
2. Structure Adjustment in the ABAP Dictionary
    ...
    - 테이블 내용은 삭제됨 (데이터 유실)
    1. ALTER TABLE 로 구조 변경
    - DB 테이블 정의를 직접 변경
    - 기존 데이터는 유지
    - 경우에 따라 인덱스는 재생성해야 할 수 있음
    1. 테이블을 변환(conversion)
    - 가장 시간이 많이 소요되는 방식
...
3. The Table Conversion Process : 테이블을 변환할 때 SAP 시스템이 어떤 절차를 사용하는지는 다음 요소에 따라 달라진다.
...
1. 실습 2 - Scarr - Copy 구조체 ZTSCARR_G01 → Append Structures 추가하기
...
7. **Database View 실습 → 노션 참고**
8. **Projection View 실습 → 노션 참고**
9. **Maintenance View 실습 → 노션 참고**
